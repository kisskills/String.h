...
s21_size_t find_udigit(unsigned long number);
...
void uint_to_char(char *copy, unsigned long number);
...
int s21_sprintf(char *str, const char *format, ...) {
    ...
    while (*format) {
        if (*format != '%') {
            ...
            if (!check_string(
                    format)) {  // проверяет всю строку после символа % на
            ...
            format +=
                counter;  // размер сдвига рассчитывается в s21_parser, когда мы
                          //  заполняем буффер
            counter = 0;
        }
    }
...
void read_nostar_width_pres(char *token_str, P_Token *t) {
    ...
    while (*token_str) {
        if (is_dec(*token_str) && *(token_str - 1) != '.') {
            t->width = !p_flag.minuses ? s21_atoi(token_str)
                                       : s21_atoi(token_str) * (-1);
...
void processing_d_and_u(char *str, char *token_str, va_list arg_ptr, P_Token t,
    ...
    switch (ch) {
        ...
        case 'u':
            if (p_flag.length) {
                if (token_str[token_len - 2] == 'l') {
                    u_number = va_arg(arg_ptr, unsigned long int);
                }
                if (token_str[token_len - 2] == 'h') {
    ...
    if (t.precision == 0 && p_flag.precision == 1 && d_number == 0 &&
        ...
        uint_to_char(copy, (unsigned long int)u_number);
        add_zero_prec_d(copy, t);
    }
...
void add_zero_prec_d(char *copy, P_Token t) {
    ...
    if (t.precision) {
        if (is_dec(*copy)) {
            ...
            // ++t.precision;
            s21_memmove(copy + t.precision + 1, copy + 1, len);
...
void uint_to_char(char *copy, unsigned long number) {
    s21_size_t digit = find_udigit(number);
...
void string_to_char(char *copy, char *tmp, P_Token t) {
    if (t.precision != 0) {
        if (t.precision > (int)s21_strlen(tmp)) {
            s21_strcpy(copy, tmp);
        } else {
